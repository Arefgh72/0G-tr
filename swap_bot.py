...
FEE_TIERS_TO_TRY = [3000, 500, 10000, 100]  # لیست کارمزدهای احتمالی برای تست
...
def perform_round_trip_swap():
    print("--- 🤖 شروع به کار ربات سواپ رفت و برگشتی (نسخه تست واقعی) ---")
    ...
    # <<-- حلقه تست کارمزدهای مختلف با ارسال تراکنش واقعی -->>
    for fee in FEE_TIERS_TO_TRY:
        try:
            print(f"  --- تلاش با کارمزد {fee} ---")
            params = (
                Web3.to_checksum_address(W_OG_ADDRESS),
                Web3.to_checksum_address(EUCLID_TOKEN_ADDRESS),
                fee,
                account.address,
                int(time.time()) + 600,
                amount_in_wei,
            ...
            print(f"    تراکنش تست ارسال شد: {tx_hash.hex()}")
            ...
            if receipt.status == 1:
                ...
                break # از حلقه کارمزدها خارج شو
            else:
                print(f"    کارمزد {fee} ناموفق بود (reverted).")
        
        except Exception as e:
            print(f"    خطا در تست کارمزد {fee}: {e}")
            time.sleep(5) # کمی تاخیر قبل از تست بعدی

    if successful_fee is None:
        print("🚨 هیچ کارمزد معتبری پیدا نشد. پایان عملیات.")
        return

    # اگر سواپ اول موفق بود، ادامه می‌دهیم
    if amount_received <= 0:
        print("🚨 هیچ توکنی دریافت نشد. پایان عملیات.")
        return
    ...
    try:
        ...
        # مرحله ب: خود سواپ (با همان کارمزد موفق قبلی)
        params_reverse = (Web3.to_checksum_address(EUCLID_TOKEN_ADDRESS), Web3.to_checksum_address(W_OG_ADDRESS), successful_fee, account.address, int(time.time()) + 600, amount_received, 0, 0)
        nonce = w3.eth.get_transaction_count(account.address)
